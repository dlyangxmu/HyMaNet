...
@time: 2022,9月
@modified_time: 2024,11月
Copyright (c), xiaohongshu,Kunming Medical University
...
from model_others.HyMaNet import GCN_EAI
from model_others.HyMaNet import MAM_XCI
...
def main(opt, rank, local_rank, world_size, device):
    ...
    train_dataset = Grab(path_to_data=opt.grab_data_dict, input_n=input_n, output_n=output_n, split=0,
                         debug=opt.is_debug, using_saved_file=opt.is_using_saved_file,
                         using_noTpose2=opt.is_using_noTpose2)
    print(">>> loading val_data")
    val_dataset = Grab(path_to_data=opt.grab_data_dict, input_n=input_n, output_n=output_n, split=1, debug=opt.is_debug,
                       using_saved_file=opt.is_using_saved_file, using_noTpose2=opt.is_using_noTpose2)
    ...
    model = MAM_XCI(input_feature=all_n, hidden_feature=opt.linear_size, p_dropout=opt.dropout, num_stage=opt.num_stage,
                    lh_node_n=opt.num_lh * 3, rh_node_n=opt.num_rh * 3, b_node_n=opt.num_body * 3)
    ...
    if is_cuda:
        ...
        model = DDP(model, device_ids=[local_rank], output_device=local_rank,
                    find_unused_parameters=True)  # [*] DDP(...)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        # 训练
        Ir_now, t_l, = train(epoch,train_loader, model, optimizer, device=device, lr_now=lr_now, max_norm=opt.max_norm,
                             dct_trans=dct_trans, idct_trans=idct_trans, is_boneloss=opt.is_boneloss,
                             is_weighted_jointloss=opt.is_weighted_jointloss)
        # 训练结果
        ...
        # 验证
        v_loss = validate(val_loader, model, device=device, dct_trans=dct_trans, idct_trans=idct_trans,is_boneloss=opt.is_boneloss,is_weighted_jointloss=opt.is_weighted_jointloss)
        # 短时结果
...
def train(epoch,train_loader, model, optimizer, device, lr_now, max_norm, dct_trans, idct_trans, is_boneloss,
          is_weighted_jointloss):
    ...
    for (input_pose, target_pose) in tqdm(train_loader):
        ...
        pred_3d = out_pose
        targ_3d = target_pose
        # pred_3d, targ_3d = idct_trans(y_out=out_pose, out_joints=target_pose1, device=device)
...
def validate(val_loader, model, device, dct_trans, idct_trans,is_boneloss,
          is_weighted_jointloss):
...
def rotate_data(data, angle, axis):
    # 根据选择的轴进行旋转
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            data[i, j, :, :] = np.dot(data[i, j, :, :], rotation_matrix)
