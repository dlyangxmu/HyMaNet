...
def main(opt):
    ...
    model = MAM_XCI(input_feature=all_n, hidden_feature=opt.linear_size, p_dropout=opt.dropout, num_stage=opt.num_stage, lh_node_n=opt.num_lh*3, rh_node_n=opt.num_rh*3,b_node_n=opt.num_body*3)
...
def joint_loss_FDE(y_out, out_poses):
    y_out = y_out.reshape(-1, 3)
    out_poses = out_poses.reshape(-1, 3)
    return torch.mean(torch.sqrt(torch.sum((y_out - out_poses) ** 2, dim=-1)))

def joint_loss_rmse(y_out, out_poses):
    y_out = y_out.reshape(-1, 3)
    out_poses = out_poses.reshape(-1, 3)
    return torch.sqrt(torch.mean((y_out - out_poses) ** 2))
...
def test_split(test_loader, model, device, dct_trans,idct_trans):
    ...
    with torch.no_grad():
        for i, (input_pose, target_pose) in enumerate(test_loader):
            ...
            pred_3d, targ_3d = out_pose, target_pose
            ...
            for k in np.arange(0, len(eval_frame)):  
                ...
                loss_wholebody = joint_loss_rmse(test_out, test_joints)
                t_posi[k] += loss_wholebody.cpu().data.numpy() * n * 100

                test_body_out, test_body_joints = pred_3d[:, j, :25, :], targ_3d[:, j, :25, :]
                t_body_posi[k] += joint_loss_rmse(test_body_out, test_body_joints).cpu().data.numpy() * n * 100

                test_lhand_out, test_lhand_joints = pred_3d[:, j, 25:40, :], targ_3d[:, j, 25:40, :]
                t_lhand_posi[k] += joint_loss_rmse(test_lhand_out, test_lhand_joints).cpu().data.numpy() * n * 100

                test_rhand_out, test_rhand_joints = pred_3d[:, j, 40:, :], targ_3d[:, j, 40:, :]
                t_rhand_posi[k] += joint_loss_rmse(test_rhand_out, test_rhand_joints).cpu().data.numpy() * n * 100

                test_lhand_rel_out, test_lhand_rel_joints = rel_pred_3d[:, j, 25:40, :], rel_targ_3d[:, j, 25:40, :]
                t_lhand_rel_posi[k] += joint_loss_rmse(test_lhand_rel_out, test_lhand_rel_joints).cpu().data.numpy() * n * 100

                test_rhand_rel_out, test_rhand_rel_joints = rel_pred_3d[:, j, 40:, :], rel_targ_3d[:, j, 40:, :]
                t_rhand_rel_posi[k] += joint_loss_rmse(test_rhand_rel_out, test_rhand_rel_joints).cpu().data.numpy() * n * 100
