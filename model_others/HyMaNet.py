...
class GCN_EAI(nn.Module):
    def __init__(self, input_feature=165, hidden_feature=256, p_dropout=0.5, num_stage=12, lh_node_n=15*3, rh_node_n=15*3,b_node_n=25*3):
...
class GSU(nn.Module):
    ...
    def forward(self, task_j_features, task_k_features):
        ...
        # 返回融合后的特征
        return task_j_features + selected_features

class MAM_XCI(nn.Module):
    def __init__(self, input_feature=60, hidden_feature=256, p_dropout=0.5, num_stage=12, lh_node_n=15*3, rh_node_n=15*3,b_node_n=25*3):
        ...
        # 门控选择单元
        self.gsu = GSU(input_feature)

        # Individual Encoder
        num_stage_encoder = 12
        self.whole_encoder1 = SelfAttention_block(input_feature, hidden_feature, p_dropout, num_stage_encoder, node_n=165)
        self.whole_encoder2 = SelfAttention_block(input_feature, hidden_feature, p_dropout, num_stage_encoder, node_n=165)
        self.whole_encoder3 = SelfAttention_block(input_feature, hidden_feature, p_dropout, num_stage_encoder, node_n=165)
    ...
    def forward(self, inputdata, action=None, pde_ml=None,pde_lm=None,pde_mr=None,pde_rm=None,pde_lr=None,pde_rl=None):

        # data process & wrist replicate
        b1, f1, n1, ax1 = inputdata.shape
        x = inputdata.permute(0, 2, 3, 1).view(b1, -1, f1)

        whole_body_ = self.whole_encoder1(x)

        whole_body_1 = self.mamba(whole_body_)

        x1 = whole_body_

        b, n, f = x1.shape
        whole_body_x = x1.view(b, -1, 3, f)
        lwrist = whole_body_x[:,20:21].detach()
        rwrist = whole_body_x[:,21:22].detach()
        b_x = whole_body_x[:,:25].view(b, -1, f)
        lh_x = torch.cat((lwrist,whole_body_x[:,25:40]),1)
        ...
        for i in range(self.ca_num_layers):
            rhand_2_hbody = self._inter_body_rhand_layers[i](rhand_2_hbody, rhand)
            lhand_2_hbody = self._inter_body_lhand_layers[i](lhand_2_hbody, lhand)

            lhand_2_rhand = self._inter_rhand_lhand_layers[i](lhand_2_rhand, lhand)
            hbody_2_rhand = self._inter_rhand_body_layers[i](hbody_2_rhand, hbody)

            rhand_2_lhand = self._inter_lhand_rhand_layers[i](rhand_2_lhand, rhand)
            hbody_2_lhand = self._inter_lhand_body_layers[i](hbody_2_lhand, hbody)
        ...
        fusion_lwrist = fusion_lwrist.view(b, -1, 3, f)
        fusion_rwrist = fusion_rwrist.view(b, -1, 3, f)
        whole_body_2 = torch.cat([hbody_no_wrist[:,:20],fusion_lwrist,fusion_rwrist,hbody_no_wrist[:,20:],lhand_no_wrist,rhand_no_wrist],1).view(b, -1, f)

        # 使用门控单元共享特征
        task1_output = self.gsu(whole_body_1, whole_body_2)
        output2 = self.whole_encoder3(task1_output)

        return output2.permute(0, 2, 1).view(b, f, -1, 3)
